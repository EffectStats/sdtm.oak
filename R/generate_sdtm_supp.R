...
generate_sdtm_supp <- function(sdtm_dataset,
  ...
  admiraldev::assert_character_scalar(eval_var)
  
  admiraldev::assert_data_frame(supp_qual_info, required_vars = rlang::syms(c(qnam_var, label_var, orig_var, eval_var)))
  ...
  supp_qual_info <- supp_qual_info |>
    dplyr::distinct(dplyr::across(dplyr::all_of(c(qnam_var, label_var, orig_var, eval_var))), .keep_all = TRUE)
